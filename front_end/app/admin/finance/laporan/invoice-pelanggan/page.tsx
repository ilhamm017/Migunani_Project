...
export default function LaporanInvoicePelangganPage() {
  ...
  const load = useCallback(async () => {
    try {
      ...
      console.error(e);
      notifyAlert('Gagal memuat invoice customer');
    } finally {
      setLoading(false);
    }
  }, []);
    ...
    <div className="bg-slate-50 min-h-screen pb-10">
      <div className="bg-white px-6 py-4 shadow-sm sticky top-0 z-40">
        <div className="flex items-center gap-3">
          ...
          <h1 className="font-bold text-lg text-slate-900">Invoice Pelanggan</h1>
        </div>
      </div>

      <div className="p-5 space-y-4">
        <div className="bg-white p-5 rounded-2xl shadow-sm border border-slate-100">
          <p className="text-xs text-slate-500 mb-1 font-bold uppercase">Outstanding Invoice (Belum Lunas)</p>
          <p className="text-3xl font-black text-slate-900">{formatCurrency(totalDue)}</p>
          <p className="text-xs text-slate-500 mt-2">
            Jumlah invoice aktif: <span className="font-bold text-slate-700">{rows.length}</span>
        ...
        <div className="bg-white p-4 rounded-2xl shadow-sm border border-slate-100 space-y-3">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
            ...
            >
              ...
              <p className="text-xs font-semibold text-slate-500 mt-1">Daftar invoice berjalan + cetak.</p>
            </Link>
            <Link
            ...
            >
              Aging Piutang (AR)
              <p className="text-xs font-semibold text-slate-500 mt-1">Ringkasan aging berdasarkan bucket.</p>
            </Link>
          </div>
        </div>
      </div>
