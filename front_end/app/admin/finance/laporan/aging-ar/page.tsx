        ...
        <div className="bg-slate-50 min-h-screen pb-10">
            <div className="bg-white px-6 py-4 shadow-sm sticky top-0 z-40 mb-4">
                <div className="flex items-center gap-3">
                    ...
                    <h1 className="font-bold text-lg text-slate-900">Aging Piutang (AR)</h1>
                </div>
            </div>
                ...
                {loading ? (
                    ...
                    <>
                        ...
                        <div className="bg-white p-5 rounded-2xl shadow-sm border border-slate-100">
                            <p className="text-xs text-slate-500 mb-1 font-bold uppercase">Total Piutang Customer</p>
                                ...
                                {data.details?.map((item: any) => (
                                    <div key={item.id} className="bg-white p-4 rounded-xl shadow-sm border border-slate-100">
                                        <div className="flex justify-between items-start mb-2">
                                            <div>
                                                <h4 className="font-bold text-slate-900 text-sm">{item.customer}</h4>
                                                <p className="text-xs text-slate-500 font-mono">{item.invoice_number}</p>
                                            </div>
                                            <div className="text-right">
                                                <p className="font-bold text-rose-600 text-sm">{formatCurrency(item.amount_due)}</p>
                                                <p className="text-[10px] text-slate-400">{new Date(item.created_at).toLocaleDateString()}</p>
                                            </div>
                                        </div>

                                        <div className="mt-2 flex items-center justify-between">
                                            <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Umur Piutang</span>
                                            <span className={`text-xs font-bold px-2 py-1 rounded-full ${item.days_outstanding > 90 ? 'bg-rose-100 text-rose-700' :
                                                item.days_outstanding > 30 ? 'bg-amber-100 text-amber-700' :
