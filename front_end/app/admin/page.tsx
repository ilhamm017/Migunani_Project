  ...
  const refreshOverview = useCallback(async () => {
    ...
    try {
      ...
      if (role === 'admin_finance') {
        ...
        actionableCount = Number(stats.ready_to_ship || 0) + Number(stats.checked || 0) + Number(stats.hold || 0);
      } else if (role === 'kasir') {
      ...
      if (role === 'admin_gudang' || role === 'super_admin') {
        ...
        Object.assign(newWarehouseBadges, {
          ...
          '/admin/warehouse/pesanan': processingOrders + allocatedOrders + readyToShipOrders + checkedOrders,
      ...
      if (role === 'kasir' || role === 'super_admin') {
        ...
        const pendingTasks = deposits.reduce((sum, d) => {
          ...
          const handovers = Array.isArray(d?.retur_handovers_pending) ? d.retur_handovers_pending.length : 0;
      ...
      if (role === 'admin_finance' || role === 'super_admin') {
        const [resExp] = await Promise.all([
      ...
      },
      {
        ...
        tone: 'bg-rose-100 text-rose-700 group-hover:bg-rose-700 group-hover:text-white',
	      ...
	      },
	      {
	        ...
	        title: 'Scraping Grup Order (WA)',
	        desc: 'Scrap order dari WhatsApp group untuk dibantu input ke order manual.',
      ...
      <div className="p-6 space-y-6">
        ...
        {activeToast && (
          <button
            ...
            className="fixed right-4 bottom-24 z-50 inline-flex items-center gap-2 rounded-full border border-emerald-200 bg-white px-3 py-2 text-[10px] font-black uppercase tracking-widest text-emerald-700 shadow-lg hover:bg-emerald-50"
          >
            <Bell size={16} />
        ...
        {dashboardToast && (
          <button
            type="button"
            onClick={() => {
              notifyOpen({ variant: 'info', title: 'Notifikasi', message: dashboardToast });
              dismissDashboardToast();
            }}
            className="fixed right-4 bottom-40 z-50 inline-flex items-center gap-2 rounded-full border border-slate-200 bg-white px-3 py-2 text-[10px] font-black uppercase tracking-widest text-slate-700 shadow-lg hover:bg-slate-50"
          >
            <Bell size={16} />
            <span>Notifikasi</span>
            <span className="h-2 w-2 rounded-full bg-slate-600" aria-label="Ada notifikasi" />
          </button>
        )}
      </div>
    );
      ...
      <div className="bg-slate-50 min-h-screen pb-20">
        ...
        <div className="px-6 py-6 space-y-6">
          ...
          {financeCardBadges.codSettlement > 0 && (
            <Link
              href="/admin/setoran-driver"
              className="block rounded-2xl border border-amber-200 bg-amber-50 p-4 shadow-sm hover:border-amber-300"
            >
              <div className="flex items-start justify-between gap-3">
                <div>
                  <p className="text-[10px] font-black uppercase tracking-widest text-amber-700">Notifikasi Setoran Driver</p>
                  <p className="mt-1 text-sm font-black text-slate-900">
                    {financeCardBadges.codSettlement} item setoran driver menunggu diproses
                  </p>
                  <p className="mt-1 text-[11px] font-semibold text-amber-800">
                    Terima uang COD dan/atau barang retur driver dalam satu proses.
                  </p>
                </div>
  ...
  if (user?.role === 'admin_gudang') {
    const warehouseMenus = [
      ...
      { href: '/admin/warehouse/driver-issues', title: 'Laporan Driver', desc: 'Follow-up barang kurang', icon: AlertTriangle, tone: 'bg-violet-100 text-violet-700 group-hover:bg-violet-700 group-hover:text-white', badge: warehouseCardBadges['/admin/warehouse/driver-issues'] || 0 },
	      { href: '/admin/warehouse/helper', title: 'Picklist Alokasi', desc: 'Daftar barang alokasi untuk diambil', icon: UserCheck, tone: 'bg-indigo-100 text-indigo-700 group-hover:bg-indigo-700 group-hover:text-white', badge: warehouseCardBadges['/admin/warehouse/helper'] || 0 },
    ...
    let quickActionCards = [
      ...
      { href: '/admin/riwayat-pengiriman', title: 'Riwayat Pengiriman', desc: 'Bukti checker & bukti driver ke customer.', icon: Truck, badge: 0, tone: 'bg-cyan-100 text-cyan-700 group-hover:bg-cyan-700 group-hover:text-white' },
    ...
    const featureCategories = [
      ...
      menus: [
        ...
        { href: '/admin/orders/issues', title: 'Issue Pesanan', desc: 'Kendala order di lapangan.', icon: AlertTriangle },
	        ...
	        { href: '/admin/chat/whatsapp/scraping-grup-order', title: 'WA Scraping Grup Order', desc: 'Scrap order grup untuk dibantu input manual.', icon: FileSpreadsheet },
    ...
    <div className="p-4 max-[360px]:p-3 sm:p-6 pb-20 space-y-6 sm:space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
      ...
      {financeCardBadges.codSettlement > 0 && (
        ...
        >
          <div className="flex items-start justify-between gap-3">
            <div>
              <p className="text-[10px] font-black uppercase tracking-[0.22em] text-amber-700">Notifikasi Setoran Driver</p>
              <p className="mt-1 text-sm font-black text-slate-900">
                {financeCardBadges.codSettlement} item setoran driver menunggu diproses
              </p>
              <p className="mt-1 text-[11px] font-semibold text-amber-800">
