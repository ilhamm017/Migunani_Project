...
export type ConfirmPopupVariant = 'success' | 'warning' | 'error' | 'info';
...
const VARIANT_META: Record<ConfirmPopupVariant, { icon: typeof CheckCircle2; tone: string; badge: string }> = {
      ...
      >
        <div className="flex items-start justify-between gap-3">
          <div className="flex items-start gap-3">
            ...
            <div>
              <p className="text-[10px] font-black uppercase tracking-[0.22em] text-slate-500">Konfirmasi</p>
        ...
        <div className="mt-4 flex gap-2">
          {secondaryLabel ? (
            <button
              ...
              className="flex-1 rounded-2xl bg-white px-4 py-3 text-sm font-black text-slate-700 shadow-sm hover:bg-slate-50 disabled:opacity-60"
            >
              {secondaryLabel}
            </button>
          ) : null}
          <button
            ...
            className="flex-1 rounded-2xl bg-slate-900 px-4 py-3 text-sm font-black text-white shadow-sm hover:bg-black disabled:opacity-60"
          >
            {primaryLabel}
          </button>
        </div>
