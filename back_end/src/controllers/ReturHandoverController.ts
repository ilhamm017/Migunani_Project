...
import { ReturHandover, ReturHandoverItem, Retur, Product, User } from '../models';
...
export const receiveReturHandover = asyncWrapper(async (req: Request, res: Response) => {
    ...
    for (const row of items) {
        ...
        if (!returId) throw new CustomError('retur_id wajib diisi', 400);
        if (!expectedReturIds.has(returId)) throw new CustomError('retur_id tidak termasuk handover ini', 409);
        if (!Number.isFinite(qtyReceived) || qtyReceived < 0) throw new CustomError('qty_received tidak valid', 400);
